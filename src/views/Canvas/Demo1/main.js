...
(function (global) {
    ...
    for (var i = 0; i < recGroup.length; i++) {
      ...
      if (Math.abs(drag.x - rec.x) < width / 2 && Math.abs(drag.y - rec.y) < height / 2) {
        ...
        if (drag.idx === audioGroup[currentFrame][count] && checkIsRight(rec.idx)) {
          ...
          global.audioPlayer.playAudioCallback(require('@/assets/demo1/sounds/wego_right.mp3'), function () {
          ...
          global.audioPlayer.playAudio(require('@/assets/demo1/sounds/wego_error.mp3'));
        }
      }
    }
    drag.x = drag.initX;
  ...
  function playNextAudio() {
    ...
    global.audioPlayer.playAudioCallback(require(`@/assets/demo1/sounds/${num}.mp3`), function () {
      playNextAudio()
    }, this);
  }
  ...
  function handleControl() {
    utils.on(cover.btnStart, 'click', function () {
      cover.visible = false;
      global.audioPlayer.playAudioCallback(require('@/assets/demo1/sounds/bg.mp3'), function () {
        playNextAudio();
      }, this);
    });
